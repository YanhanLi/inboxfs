...
Undo history is stored as a private JSON file under `~/.inboxfs/`. InboxFS is an organizer, not a backup system.
...
InboxFS binds to the loopback interface and exposes no cloud service. Mutations accept IDs from a fresh scan and re-scan before moving, which prevents stale previews from silently applying. Destinations are canonicalized before use; symlink escapes, changed undo targets, and occupied restore paths are rejected.
