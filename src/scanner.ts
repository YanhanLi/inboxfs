...
import { configFingerprint, readInboxConfig } from "./config.js";
...
function suggestionId(sourcePath: string, modifiedMs: number, size: number, destinationPath: string, rulesFingerprint: string): string {
  return createHash("sha256").update(`${sourcePath}\0${modifiedMs}\0${size}\0${destinationPath}\0${rulesFingerprint}`).digest("hex").slice(0, 16);
}

export async function scanInbox(root: string): Promise<ScanResult> {
  const canonicalRoot = await realpath(root);
  const config = await readInboxConfig(canonicalRoot);
  const rulesFingerprint = configFingerprint(config);
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    const { category, classification } = explainClassification(entry.name, config.rules, metadata.size);
    ...
    suggestions.push({
      id: suggestionId(sourcePath, metadata.mtimeMs, metadata.size, destinationPath, rulesFingerprint),
      name: entry.name,
  ...
  return {
    ...
    ruleConfig: { version: 2, customRuleCount: config.rules.length, source: config.source, migratedFromVersion: config.migratedFromVersion }
  };
}
